...
source "proxmox-iso" "wsservernogui22" {
  node                     = var.node
  ...
  additional_iso_files {
    device   = "sata5"
    iso_file = "local:iso/templatewsnogui22.iso"
    unmount  = true
  }
  network_adapters {
    bridge = "vmbr0"
  ...
  winrm_use_ssl        = true
  winrm_timeout        = "1h"
  template_description = "WindowsServer22NoGui"
  template_name        = "templatewsnogui22"
}

build {
  sources = ["source.proxmox-iso.wsservernogui22"]
